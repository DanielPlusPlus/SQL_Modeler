...
class LoadPostgreSQLController(LoadSQLControllerInterface):
    ...
    def __mapToOracleType(self, pgType: str) -> str:
        if not pgType:
            return pgType
        upper_type = pgType.upper().strip()
        ...
        for line in raw_lines:
            ...
            if upper.startswith("CONSTRAINT") or upper.startswith("PRIMARY KEY") or upper.startswith("UNIQUE") or upper.startswith("FOREIGN KEY"):
                ...
                if m_fk:
                    ...
                    ref_table = m_fk.group(2).strip().strip('"')
                    ref_cols_raw = m_fk.group(3)

                    local_cols = [c.strip().strip('"') for c in local_cols_raw.split(',')]
                    ref_cols = [c.strip().strip('"') for c in ref_cols_raw.split(',')]
            ...
            data_type_raw = m_col.group("type").strip()
    ...
    def __parseRelationshipSQL(self, sql: str):
        ...
        second_column_name = m_fk.group(1).split(',')[0].strip().strip('"')
        first_table_name = m_ref.group(1).strip().strip('"')
        first_column_name = m_ref.group(2).split(',')[0].strip().strip('"')
