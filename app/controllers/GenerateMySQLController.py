...
class GenerateMySQLController(GenerateSQLControllerInterface):
    ...
    def _generateCreateTableSQLCode(self, ObtainedTable):
        ...
        for column in tableColumns:
            mysqlType = self.__mapDataType(column["dataType"])
            columnSQL = f'`{column["columnName"]}` {mysqlType}'
            if column["length"]:
    ...
    def _generateCreateJunctionTableSQLCode(self, ObtainedRelationship):
        ...
        createJunctionTableSQL = (
            ...
            f'    `{firstTableName}_{firstSelectedColumnName}` INT NOT NULL,\n'
            f'    `{secondTableName}_{secondSelectedColumnName}` INT NOT NULL,\n'
