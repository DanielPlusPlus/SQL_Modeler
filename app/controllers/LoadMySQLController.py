...
class LoadMySQLController(LoadSQLControllerInterface):
    ...
    def __mapToOracleType(self, mysqlType: str) -> str:
        if not mysqlType:
            return mysqlType
        upper_type = mysqlType.upper()
        upper_type = upper_type.split('(')[0].strip()
        return self.__DATA_TYPE_REVERSE_MAP.get(upper_type, upper_type)
    ...
    def __parseCreateTableSQL(self, sql: str):
        ...
        for line in raw_lines:
            ...
            if upper.startswith("CONSTRAINT") or upper.startswith("PRIMARY KEY") or upper.startswith("UNIQUE") or upper.startswith("FOREIGN KEY"):
                ...
                if m_pk:
                    cols = [c.strip().strip('`').strip('"') for c in m_pk.group(1).split(',')]
                    for c in cols:
                        pk_columns.add(c)

                m_uniq = re.search(r'UNIQUE\s+KEY\s+`[^`]+`\s*\(([^)]+)\)', line, re.IGNORECASE)
                if not m_uniq:
                    m_uniq = re.search(r'UNIQUE\s*\(([^)]+)\)', line, re.IGNORECASE)
                if m_uniq:
                    cols = [c.strip().strip('`').strip('"') for c in m_uniq.group(1).split(',')]
                    for c in cols:
                ...
                if m_fk:
                    ...
                    local_cols = [c.strip().strip('`').strip('"') for c in local_cols_raw.split(',')]
                    ref_cols = [c.strip().strip('`').strip('"') for c in ref_cols_raw.split(',')]
    ...
    def __parseRelationshipSQL(self, sql: str):
        ...
        second_column_name = m_fk.group(1).strip().strip('`').strip('"')
        first_table_name = m_ref.group(1)
        first_column_name = m_ref.group(2).split(',')[0].strip().strip('`').strip('"')
